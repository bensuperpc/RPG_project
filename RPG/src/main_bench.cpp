...
int main()
{
    ...
    const std::vector<std::pair<const std::string, bool (*)(const long long int &)>> pointer_map {{"isPrime_opti_1", &math::prime::isPrime_opti_1},
        ...
        {"isPrime_opti_6", &math::prime::isPrime_opti_6}, {"isPrime_opti_8(1&5)", &math::prime::isPrime_opti_8}};
...
#if __cplusplus >= 201703L
    thread::Pool thread_pool2(std::thread::hardware_concurrency() / 2);
#else
    ThreadPool thread_pool2(std::thread::hardware_concurrency() / 2);
#endif

    std::vector<int> nbrs_nbrs(1000);
